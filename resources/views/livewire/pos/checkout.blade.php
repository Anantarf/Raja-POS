<div class="h-screen flex flex-col overflow-hidden bg-[#F3F6F4] font-sans text-[#232E28]">
    <!-- Topbar Navigation Header (100% Identical Sizing & Spacing with Admin Topbar) -->
    ...
    <!-- Main Operational Split View (100% Identical Padding px-7 pb-7 pt-2 with Admin Main) -->
    <div class="flex-1 flex overflow-hidden px-7 pb-7 pt-2 gap-6">

        <!-- LEFT COLUMN: Product Catalog (65%) -->
        <div class="w-full lg:w-[65%] flex flex-col flex-shrink-0">

            <!-- Search Bar & Filters Header (Identical Sizing with Admin Toolbar) -->
            <div class="p-5 bg-white rounded-3xl border border-[#E3EEE8] space-y-3.5 mb-4">
                <!-- Search Input -->
                <div class="relative">
                    <input
                        type="text"
                        ...
                        class="w-full pl-12 pr-10 py-3 text-sm font-semibold border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#3F7A5D]/20 focus:border-[#3F7A5D] bg-[#F3F6F4] placeholder:text-[#718379] transition-all"
                        autofocus
                    />
                ...
                <div class="flex items-center justify-between gap-3 text-xs pt-0.5">
                    ...
                    <div class="flex items-center gap-2 overflow-x-auto py-1 px-0.5 shrink min-w-0">
                        <button
                            ...
                            class="px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 border {{ $selectedCategory === null ? 'bg-[#3F7A5D] text-white border-[#3F7A5D]' : 'bg-[#F3F6F4] text-[#232E28] border-slate-200 hover:bg-slate-200' }}"
                        >
                            Semua Kategori
                        </button>
                        @foreach($categories as $cat)
                            <button
                                ...
                                class="px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 border {{ $selectedCategory === $cat->id ? 'bg-[#3F7A5D] text-white border-[#3F7A5D]' : 'bg-[#F3F6F4] text-[#232E28] border-slate-200 hover:bg-slate-200' }}"
                            >
                                {{ $cat->name }}
                            </button>
                        @endforeach
            ...
            <!-- Product Cards Grid (Identical Card Scale with Admin Cards) -->
        ...
        <div class="w-full lg:w-[35%] bg-white rounded-3xl border border-[#E3EEE8] flex flex-col flex-shrink-0 overflow-hidden h-full">
            ...
            <!-- 2. FLEX-1 CART ITEMS LIST (Maximized Vertical Space) -->
            <div class="flex-1 overflow-y-auto px-6 py-3 divide-y divide-slate-100 min-h-0">
                ...
                @empty
                    <div class="h-full min-h-[160px] flex flex-col items-center justify-center text-center text-slate-400 space-y-2 p-4">
            ...
            <!-- 3. COMPACT HIGH-DENSITY PAYMENT FOOTER -->
            <div class="p-5 border-t border-[#E3EEE8] bg-[#F3F6F4]/50 space-y-3 shrink-0">
                ...
                <div class="bg-white p-4 rounded-2xl border border-[#E3EEE8] flex items-baseline justify-between">
                    <div>
                        ...
                        <div class="text-xs text-slate-400 font-medium">Subtotal: Rp {{ number_format($this->subtotal, 0, ',', '.') }}</div>
                    </div>
                ...
                @if(count($cart) > 0)
                    <div class="flex items-center gap-1.5 overflow-x-auto text-xs font-bold py-0.5">
                        ...
                        <button wire:click="setPaymentAmount(10000)" class="px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl shrink-0 hover:bg-slate-100 text-[#232E28]">10rb</button>
                        <button wire:click="setPaymentAmount(20000)" class="px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl shrink-0 hover:bg-slate-100 text-[#232E28]">20rb</button>
                        <button wire:click="setPaymentAmount(50000)" class="px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl shrink-0 hover:bg-slate-100 text-[#232E28]">50rb</button>
                        <button wire:click="setPaymentAmount(100000)" class="px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl shrink-0 hover:bg-slate-100 text-[#232E28]">100rb</button>
                    </div>
                @endif
                ...
                <!-- Cash Change Display -->
                <div class="bg-white px-4 py-2.5 rounded-2xl border border-[#E3EEE8] flex items-center justify-between text-xs">
                    <span class="font-extrabold text-[#232E28]">Kembalian</span>
                    <span class="{{ $this->change_amount > 0 ? 'text-emerald-700 font-mono text-lg font-extrabold' : 'text-[#232E28] font-mono text-xs font-bold' }}">
                ...
                <button
                    ...
                    class="w-full py-3.5 rounded-2xl font-extrabold text-xs text-white uppercase tracking-wider transition-all {{ count($cart) > 0 && $this->total_paid >= $this->grand_total ? 'bg-[#3F7A5D] hover:bg-[#32634B] cursor-pointer' : 'bg-[#E3EEE8] text-[#718379] cursor-not-allowed border border-slate-200' }}"
                >
