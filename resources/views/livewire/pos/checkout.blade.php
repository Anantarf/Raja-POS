<div class="h-screen flex flex-col overflow-hidden bg-[#F3F6F4] font-sans text-[#232E28]">
    ...
    <div class="flex-1 flex overflow-hidden px-5 pb-4 gap-4">
        ...
        <div class="w-full lg:w-[65%] flex flex-col flex-shrink-0">
            ...
            <!-- Compact Streamlined Toolbar (Fix Catatan B: Compact Filter Header) -->
            <div class="p-3 bg-white rounded-2xl border border-[#E3EEE8] space-y-2 mb-2.5">
                <!-- Search Input -->
                <div class="relative">
                    <input
                        type="text"
                        ...
                        class="w-full pl-10 pr-9 py-2 text-xs font-semibold border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3F7A5D]/20 focus:border-[#3F7A5D] bg-[#F3F6F4] placeholder:text-[#718379] transition-all"
                        autofocus
                    />
                    ...
                    @if($search)
                        <button wire:click="$set('search', '')" class="absolute right-3 top-2 text-slate-400 hover:text-slate-600 text-xs font-bold bg-slate-200 rounded-full w-5 h-5 flex items-center justify-center">
                            &times;
                        </button>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-between gap-2 text-xs">
                    <!-- Category Tabs -->
                    <div class="flex items-center gap-1.5 overflow-x-auto py-0.5 px-0.5 shrink min-w-0">
                        <button
                            wire:click="$set('selectedCategory', null)"
                            class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 border {{ $selectedCategory === null ? 'bg-[#3F7A5D] text-white border-[#3F7A5D]' : 'bg-[#F3F6F4] text-[#232E28] border-slate-200 hover:bg-slate-200' }}"
                        >
                            Semua Kategori
                        </button>
                        @foreach($categories as $cat)
                            <button
                                ...
                                class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 border {{ $selectedCategory === $cat->id ? 'bg-[#3F7A5D] text-white border-[#3F7A5D]' : 'bg-[#F3F6F4] text-[#232E28] border-slate-200 hover:bg-slate-200' }}"
                            >
                                {{ $cat->name }}
                            </button>
                        @endforeach
                    ...
                    <!-- Type Filter Pills -->
                    <div class="flex items-center gap-1 shrink-0 text-xs font-bold border-l border-[#E3EEE8] pl-2.5">
                        <button
            ...
            <!-- Product Cards Grid (Fix Catatan A: 3 Kolom yang Sangat Lapang & Nyaman) -->
            <div class="flex-1 overflow-y-auto pr-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                @forelse($products as $product)
                    ...
                    <div
                        ...
                        class="bg-white border border-[#E3EEE8] hover:border-[#3F7A5D]/60 hover:bg-[#F3F6F4]/50 rounded-2xl p-3.5 flex flex-col justify-between cursor-pointer transition-all duration-200 relative group {{ $isIncomplete ? 'opacity-65 bg-rose-50/20' : '' }}"
                    >
                        <div>
                            <!-- Top Metadata Row -->
                            ...
                            <!-- Product Thumbnail & Title (Spacious 3-Column Layout) -->
        ...
        <div class="w-full lg:w-[35%] bg-white rounded-2xl border border-[#E3EEE8] flex flex-col flex-shrink-0 overflow-hidden h-full">
            ...
            <!-- 2. MAXIMIZED FLEX-1 CART ITEMS LIST -->
            <div class="flex-1 overflow-y-auto px-4 py-2 divide-y divide-slate-100 min-h-0">
                ...
                @empty
                    <div class="h-full min-h-[140px] flex flex-col items-center justify-center text-center text-slate-400 space-y-1.5 p-4">
                        ...
                        <div class="font-bold text-[#232E28] text-xs">Keranjang Kosong</div>
            ...
            <div class="p-3.5 border-t border-[#E3EEE8] bg-[#F3F6F4]/50 space-y-2.5 shrink-0">
                ...
                <!-- Fix Catatan C: Clear & High-Contrast Quick Cash Shortcut Pills -->
                @if(count($cart) > 0)
                    <div class="flex items-center gap-1.5 overflow-x-auto text-xs font-extrabold py-0.5">
                        ...
                        <button wire:click="setExactPayment" class="px-3 py-1.5 bg-[#3F7A5D] text-white rounded-xl shrink-0 hover:bg-[#32634B] font-extrabold shadow-sm transition">Uang Pas</button>
                        <button wire:click="setPaymentAmount(10000)" class="px-2.5 py-1.5 bg-white border border-[#3F7A5D]/30 text-[#3F7A5D] hover:bg-[#E3EEE8] rounded-xl shrink-0 font-extrabold transition">10rb</button>
                        <button wire:click="setPaymentAmount(20000)" class="px-2.5 py-1.5 bg-white border border-[#3F7A5D]/30 text-[#3F7A5D] hover:bg-[#E3EEE8] rounded-xl shrink-0 font-extrabold transition">20rb</button>
                        <button wire:click="setPaymentAmount(50000)" class="px-2.5 py-1.5 bg-white border border-[#3F7A5D]/30 text-[#3F7A5D] hover:bg-[#E3EEE8] rounded-xl shrink-0 font-extrabold transition">50rb</button>
                        <button wire:click="setPaymentAmount(100000)" class="px-2.5 py-1.5 bg-white border border-[#3F7A5D]/30 text-[#3F7A5D] hover:bg-[#E3EEE8] rounded-xl shrink-0 font-extrabold transition">100rb</button>
                    </div>
                @endif
