<div class="space-y-5">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-extrabold text-[#232E28] tracking-tight">Merk / Brand Produk</h1>
    ...
    <div class="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between text-sm">
    ...
    @if($showModal)
        <div class="fixed inset-0 bg-[#232E28]/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl space-y-4 border border-slate-100">
                <h3 class="text-lg font-extrabold text-[#232E28]">{{ $editingBrandId ? 'Edit Brand' : 'Tambah Brand' }}</h3>
                <form wire:submit.prevent="saveBrand" class="space-y-4 text-sm">
                    ...
                    <div class="flex gap-2 pt-2">
                        ...
                        <button type="button" wire:click="$set('showModal', false)" class="h-11 py-2.5 px-4 bg-slate-100 hover:bg-slate-200 text-[#232E28] font-bold rounded-xl text-sm transition">Batal</button>
                    </div>
                </form>
            </div>
        </div>
