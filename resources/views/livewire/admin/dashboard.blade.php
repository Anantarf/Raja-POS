<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-[#3F7A5D]/10 via-[#3F7A5D]/5 to-white border border-slate-200/80 rounded-2xl p-6 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-5 shadow-sm">
        <div class="space-y-2.5">
            ...
            <p class="text-lg text-[#3F5148] max-w-3xl leading-relaxed font-medium">
            ...
            <div class="pt-2">
                <a href="/pos" class="h-14 px-7 text-lg bg-[#3F7A5D] hover:bg-[#32634B] text-white font-extrabold rounded-2xl inline-flex items-center gap-2 transition active:scale-95 shadow-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 hover:border-[#3F7A5D]/50 shadow-sm hover:shadow-md transition-all duration-200">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[0.95rem] text-[#5F7167] font-black uppercase tracking-wide">Total Omzet</span>
            ...
            <div class="text-base text-[#5F7167] mt-2 font-medium">Transaksi Selesai</div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 hover:border-[#C2AC7C]/50 shadow-sm hover:shadow-md transition-all duration-200">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[0.95rem] text-[#5F7167] font-black uppercase tracking-wide">Margin Toko</span>
            ...
            @if(auth()->user()->hasRole('OWNER') || auth()->user()->hasPermission('report.profit.view') || auth()->user()->can('report.profit.view'))
                ...
                <div class="text-base text-[#5F7167] mt-2 font-medium">Omzet dikurangi Modal</div>
            @else
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 hover:border-[#3F7A5D]/50 shadow-sm hover:shadow-md transition-all duration-200">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[0.95rem] text-[#5F7167] font-black uppercase tracking-wide">Total Saldo Toko</span>
            ...
            <div class="text-base text-[#5F7167] mt-2 font-medium">Saldo riil seluruh akun &amp; cash</div>
        </div>

        <!-- 4. Total Transaksi -->
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 hover:border-[#3F7A5D]/50 shadow-sm hover:shadow-md transition-all duration-200">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[0.95rem] text-[#5F7167] font-black uppercase tracking-wide">Total Transaksi</span>
                <span class="px-3 py-1 rounded-lg text-xs font-bold text-[#3F7A5D] bg-[#E3EEE8]">
                    Sukses
                </span>
            </div>
            <div class="text-3xl sm:text-4xl font-extrabold text-[#232E28] font-mono tracking-tight mt-2">
                {{ number_format($metrics['sales_count'], 0, ',', '.') }} <span class="text-lg text-[#5F7167] font-normal">Trx</span>
            </div>
            <div class="text-base text-[#5F7167] mt-2 font-medium">Transaksi berhasil diproses</div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-base text-[#5F7167] font-medium mt-0.5">Visualisasi tren omzet ritel toko 7 hari terakhir.</p>
                </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4 flex flex-col justify-between">
            <div>
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-200/80">
                    <table class="w-full text-[1.05rem] text-left">
                        ...
                        <tbody class="divide-y divide-slate-100 font-medium">
                            @forelse($dailyTrends['labels'] as $index => $label)
                                <tr class="hover:bg-[#F3F6F4]/60 transition">
                                    <td class="py-3.5 px-4 font-bold text-[#232E28] text-[1.05rem]">{{ $label }}</td>
