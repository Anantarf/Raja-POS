<div class="space-y-5">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Kategori Produk</h1>
            <p class="text-xs sm:text-sm text-slate-500 font-medium mt-0.5">Kelola pengelompokan jenis barang toko.</p>
        </div>
        <button wire:click="openModal" class="h-11 px-4 bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold rounded-xl text-sm transition flex items-center gap-2 shadow-2xs active:scale-95 cursor-pointer shrink-0">
    ...
    <div class="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/90 shadow-2xs flex items-center justify-between">
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl shadow-2xs overflow-hidden">
            ...
            <tbody wire:loading.remove.delay class="divide-y divide-slate-100 font-medium">
                @forelse($categories as $cat)
                    <tr class="hover:bg-slate-50/80 transition">
                        <td class="py-3.5 px-4 font-bold text-slate-900 text-sm">{{ $cat->name }}</td>
                        <td class="py-3.5 px-4 font-mono">
                            <span class="bg-slate-100 border border-slate-200 text-emerald-700 px-2.5 py-1 rounded-md font-mono font-bold text-xs">{{ $cat->slug }}</span>
                        </td>
                    ...
                    <tr>
                        <td colspan="3">
                            <x-empty-state
                                ...
                                description="Belum ada data kategori atau kata kunci pencarian Anda tidak cocok."
        ...
        <div class="p-4 border-t border-slate-200/90">{{ $categories->links() }}</div>
    </div>
    ...
    @if($showModal)
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl space-y-4 border border-slate-100">
                <h3 class="text-lg font-extrabold text-slate-900">{{ $editingCategoryId ? 'Edit Kategori' : 'Tambah Kategori' }}</h3>
                <form wire:submit.prevent="saveCategory" class="space-y-4 text-sm">
                    <div>
                        <label class="block text-slate-800 font-bold mb-1.5 text-sm">Nama Kategori *</label>
                        <input type="text" wire:model="name" class="w-full h-11 px-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-sm font-semibold text-slate-800 bg-slate-50" required />
                    </div>
                    <div class="flex gap-2 pt-2">
                        <button type="submit" class="flex-1 h-11 bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold rounded-xl text-sm transition shadow-2xs flex items-center justify-center cursor-pointer">Simpan</button>
                        <button type="button" wire:click="$set('showModal', false)" class="h-11 px-4 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl text-sm transition flex items-center justify-center cursor-pointer">Batal</button>
                    </div>
                </form>
            </div>
        </div>
