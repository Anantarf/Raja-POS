<div class="space-y-5">
    ...
    <div class="bg-white p-4 rounded-3xl border border-[#E3EEE8] shadow-emco flex items-center justify-between text-xs">
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama kategori..." class="w-full sm:w-1/3 px-4 py-2.5 border border-slate-200 rounded-2xl text-xs focus:ring-2 focus:ring-[#3F7A5D]/20 focus:border-[#3F7A5D] bg-[#F3F6F4]" />
    </div>
    ...
    <div class="bg-white border border-[#E3EEE8] rounded-3xl shadow-emco overflow-hidden">
        <table class="w-full text-xs text-left">
            <thead class="bg-[#F3F6F4] border-b border-[#E3EEE8] text-[#718379] uppercase text-[10px] font-extrabold tracking-wider">
                <tr>
                    <th class="py-4 px-5">Nama Kategori</th>
            ...
            <tbody class="divide-y divide-slate-100 font-medium">
                @forelse($categories as $cat)
                    <tr class="hover:bg-[#F3F6F4]/60 transition">
                        <td class="py-4 px-5 font-extrabold text-[#232E28] text-sm">{{ $cat->name }}</td>
                        ...
                        <td class="py-4 px-5 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button wire:click="openModal({{ $cat->id }})" class="px-3.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 border border-indigo-200/80 rounded-xl font-extrabold text-xs transition active:scale-95">Edit</button>
                                <button wire:click="deleteCategory({{ $cat->id }})" wire:confirm="Hapus kategori ini?" class="px-3.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/80 rounded-xl font-extrabold text-xs transition active:scale-95">Hapus</button>
                            </div>
                        </td>
                    </tr>
                @empty
        ...
        <div class="p-4 border-t border-[#E3EEE8]">{{ $categories->links() }}</div>
    </div>
