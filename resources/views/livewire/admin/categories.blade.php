<div class="space-y-5">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button wire:click="openModal" class="h-11 px-4 bg-[#047857] hover:bg-[#065F46] text-white font-extrabold rounded-xl text-sm transition flex items-center gap-2 shadow-sm active:scale-95 cursor-pointer shrink-0">
                ...
                @forelse($categories as $cat)
                    <tr class="hover:bg-[#F3F6F4]/60 transition">
                        ...
                        <td class="py-3.5 px-4 font-mono">
                            <span class="bg-[#F3F6F4] border border-slate-200/80 text-[#047857] px-2.5 py-1 rounded-md font-mono font-bold text-xs">{{ $cat->slug }}</span>
                        </td>
            ...
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl space-y-4 border border-slate-100">
                ...
                <form wire:submit.prevent="saveCategory" class="space-y-4 text-sm">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="w-full h-11 px-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#047857]/20 focus:border-[#047857] text-sm" required />
                    </div>
                    <div class="flex gap-2 pt-2">
                        <button type="submit" class="flex-1 h-11 bg-[#047857] hover:bg-[#065F46] text-white font-extrabold rounded-xl text-sm transition shadow-sm flex items-center justify-center cursor-pointer">Simpan</button>
