    ...
    @if($viewMode === 'card')
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
            @forelse($products as $product)
                ...
                <div class="h-[395px] bg-white rounded-3xl border border-[#E3EEE8] hover:border-[#3F7A5D]/40 transition-all duration-200 overflow-hidden flex flex-col justify-between shadow-emco hover:shadow-lg group">
                    <div class="flex flex-col min-h-0 flex-1">
                        <!-- Top Header Badges (Enterprise Golden Ratio h-12) -->
                        <div class="h-12 px-4 flex items-center justify-between gap-2 border-b border-slate-100 bg-[#F3F6F4]/50 shrink-0">
                            ...
                            <span class="px-2.5 py-1 rounded-full text-[11px] font-bold tracking-wider {{ $product->price_status === 'COMPLETE' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200' }}">
                                {{ $product->price_status === 'COMPLETE' ? 'Lengkap' : 'Belum Lengkap' }}
                            </span>
                        </div>

                        <!-- Product Thumbnail / Initials Banner (Golden Ratio Banner h-24) -->
                        <div class="h-24 bg-gradient-to-b from-[#F3F6F4] to-white relative flex items-center justify-center p-3 overflow-hidden shrink-0">
                            ...
                            @else
                                <span class="text-2xl font-mono font-extrabold text-[#3F7A5D]/90 tracking-wider">
                                    {{ strtoupper(substr($product->code, 0, 2)) }}
                                </span>
                            @endif
                        </div>

                        <!-- Product Main Info (Eye-Friendly Typography) -->
                        <div class="px-4 pt-2.5 space-y-1 shrink-0">
                            <div class="h-10 flex items-center">
                                <h3 class="font-extrabold text-[#232E28] text-xs sm:text-sm leading-snug tracking-tight line-clamp-2 overflow-hidden text-ellipsis group-hover:text-[#3F7A5D] transition-colors">
                                    {{ $product->name }}
                                </h3>
                            </div>
                            <div class="h-6 flex items-center">
                                <span class="bg-indigo-50 text-indigo-700 border border-indigo-200/80 px-2.5 py-0.5 rounded-lg font-mono font-bold text-xs truncate max-w-full">
                                    Barcode: {{ $product->effective_barcode }}
                                </span>
                            </div>
                        </div>

                        <!-- Dimensions Box: Kategori, Jenis, Merk (Eye-Friendly 12px Spacing) -->
                        <div class="px-4 pt-2 shrink-0">
                            <div class="h-[82px] bg-[#F3F6F4]/70 p-3 rounded-2xl space-y-1 text-xs font-medium text-[#232E28] border border-slate-200/60 flex flex-col justify-center overflow-hidden">
                                <div class="flex items-center justify-between">
                                    <span class="text-[#718379] font-medium">Kategori:</span>
                                    <span class="font-bold text-[#232E28] truncate ml-1">{{ $product->category?->name ?? '-' }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-[#718379] font-medium">Jenis:</span>
                                    <span class="font-bold text-indigo-600 truncate ml-1">{{ $product->product_subtype ?: '-' }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-[#718379] font-medium">Merk:</span>
                    ...
                    <!-- Card Footer: Price, Stock & Action Buttons (Enterprise Quality) -->
                    <div class="border-t border-slate-100 p-4 pt-3 bg-white space-y-2.5 shrink-0">
