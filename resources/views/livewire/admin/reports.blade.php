<div class="space-y-6">
    ...
    @if($type === 'daily_summary')
        ...
        <!-- MODAL DIALOG INPUT SALDO AKTUAL & REKAP HARIAN (COMPACT SINGLE TABLE FORM - ANTI-AI) -->
        @if($showInputModal)
            <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
                <div class="bg-white rounded-2xl max-w-4xl w-full p-5 sm:p-6 shadow-2xl border border-slate-200/90 space-y-4 my-auto transition-all">
                    
                    <!-- Modal Header -->
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div>
                            <h3 class="text-base font-extrabold text-[#2C3E35] tracking-tight">Input Saldo Aktual &amp; Rekap Harian</h3>
                            <p class="text-xs text-[#718379] font-medium mt-0.5">Tanggal Rekap: <span class="font-bold text-[#2C3E35]">{{ $formattedDate }}</span></p>
                        </div>
                        <button type="button" wire:click="closeInputModal" class="p-1.5 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-100 transition cursor-pointer" title="Tutup Modal">
                    ...
                    <!-- Audit Status Strip -->
                    @if(! $mHasDiscrepancy)
                        <div class="px-3.5 py-2 bg-emerald-50 border border-emerald-200/80 rounded-xl flex items-center justify-between text-xs text-emerald-900">
                            <div class="flex items-center gap-2 font-medium">
                                ...
                                <span>Status Audit: <strong class="font-bold text-emerald-800">Semua Saldo Sesuai</strong> (Selisih Rp 0)</span>
                            </div>
                            <span class="text-[11px] font-extrabold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-md">SIAP KUNCI</span>
                        </div>
                    @else
                        <div class="px-3.5 py-2 bg-amber-50 border border-amber-200/80 rounded-xl flex items-center justify-between text-xs text-amber-900">
                            <div class="flex items-center gap-2 font-medium">
                                ...
                                <span>Status Audit: <strong class="font-bold text-amber-900">Terdapat Selisih Saldo</strong>. Harap lengkapi catatan rekap.</span>
                            </div>
                            <span class="text-[11px] font-extrabold text-amber-800 bg-amber-100 px-2 py-0.5 rounded-md">PERLU CATATAN</span>
                        </div>
                    @endif

                    <!-- Modal Body: Single Compact Table & Inputs -->
                    <div class="space-y-3.5 max-h-[62vh] overflow-y-auto pr-1" x-data="{ openAdj: null }">
                        ...
                        <div class="border border-slate-200/80 rounded-2xl overflow-hidden shadow-xs bg-white">
                            <table class="w-full text-xs text-left">
                                <thead class="bg-[#F3F6F4] border-b border-slate-200/80 text-[#718379] uppercase text-[10px] font-extrabold tracking-wider">
                                    <tr>
                                        <th class="py-2.5 px-3.5">Akun / Kanal Saldo</th>
                                        <th class="py-2.5 px-3.5 text-right">Ekspektasi Sistem</th>
                                        <th class="py-2.5 px-3.5 text-center w-48">Input Saldo Aktual</th>
                                        <th class="py-2.5 px-3.5 text-right">Selisih</th>
                                        <th class="py-2.5 px-3.5 text-center w-28">Penyesuaian</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 font-medium">
                                    ...
                                    <tr class="hover:bg-[#F3F6F4]/50 transition">
                                        <td class="py-2.5 px-3.5">
                                            ...
                                            <div class="text-[11px] text-[#718379] font-normal">E-Wallet (HP Toko)</div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right">
                                            <div class="font-mono font-extrabold text-[#2C3E35] text-sm">Rp {{ number_format($mDanaAkhir, 0, ',', '.') }}</div>
                                            <div class="text-[10px] text-[#718379]">Awal: Rp {{ number_format((float)$danaSaldoAwal, 0, ',', '.') }}</div>
                                        </td>
                                        <td class="py-2.5 px-3.5">
                                            <div class="relative flex items-center h-9">
                                                <span class="absolute left-2.5 font-mono text-[11px] text-slate-400 select-none pointer-events-none">Rp</span>
                                                <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('danaSaldoAndroid', val ? parseInt(val) : 0);" value="{{ $danaSaldoAndroid ? number_format((float)$danaSaldoAndroid, 0, ',', '.') : '' }}" class="w-full h-full pl-7 pr-2.5 border border-slate-300 rounded-xl bg-white font-mono font-extrabold text-right text-xs text-[#2C3E35] focus:ring-2 focus:ring-[#3F7A5D]/20 focus:border-[#3F7A5D]" placeholder="0" />
                                            </div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right whitespace-nowrap">
                                            @if($mDanaSelisih == 0)
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 rounded-md border border-emerald-200/80 inline-block">Sesuai</span>
                                            @else
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold font-mono text-rose-700 bg-rose-50 rounded-md border border-rose-200/80 inline-block">{{ $mDanaSelisih > 0 ? '+' : '' }}Rp {{ number_format($mDanaSelisih, 0, ',', '.') }}</span>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-3.5 text-center">
                                            <button type="button" x-on:click="openAdj = (openAdj === 'dana' ? null : 'dana')" class="px-2 py-1 text-[11px] font-extrabold rounded-lg transition cursor-pointer {{ ($danaTopup > 0 || $danaTrx > 0) ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 hover:bg-slate-200 text-[#718379]' }}">
                                                Top Up / Trx
                                            </button>
                                        </td>
                                    </tr>
                                    <tr x-show="openAdj === 'dana'" x-cloak class="bg-slate-50/80">
                                        <td colspan="5" class="p-3 border-t border-slate-200/60">
                                            <div class="grid grid-cols-2 gap-3 max-w-md ml-auto">
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Top Up Manual (DANA)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('danaTopup', val ? parseInt(val) : 0);" value="{{ $danaTopup ? number_format((float)$danaTopup, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Trx Dipakai (DANA)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('danaTrx', val ? parseInt(val) : 0);" value="{{ $danaTrx ? number_format((float)$danaTrx, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="hover:bg-[#F3F6F4]/50 transition">
                                        <td class="py-2.5 px-3.5">
                                            ...
                                            <div class="text-[11px] text-[#718379] font-normal">Gateway (EDC / HP)</div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right">
                                            <div class="font-mono font-extrabold text-[#2C3E35] text-sm">Rp {{ number_format($mQrisExpected, 0, ',', '.') }}</div>
                                        ...
                                        <td class="py-2.5 px-3.5 text-right whitespace-nowrap">
                                            @if($mQrisSelisih == 0)
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 rounded-md border border-emerald-200/80 inline-block">Sesuai</span>
                                            @else
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold font-mono text-rose-700 bg-rose-50 rounded-md border border-rose-200/80 inline-block">{{ $mQrisSelisih > 0 ? '+' : '' }}Rp {{ number_format($mQrisSelisih, 0, ',', '.') }}</span>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-3.5 text-center">
                                            <button type="button" x-on:click="openAdj = (openAdj === 'qris' ? null : 'qris')" class="px-2 py-1 text-[11px] font-extrabold rounded-lg transition cursor-pointer {{ ($qrisTarikTunai > 0) ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 hover:bg-slate-200 text-[#718379]' }}">
                                                Tarik Tunai
                                            </button>
                                        </td>
                                    </tr>
                                    <tr x-show="openAdj === 'qris'" x-cloak class="bg-slate-50/80">
                                        <td colspan="5" class="p-3 border-t border-slate-200/60">
                                            <div class="max-w-xs ml-auto">
                                                <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Tarik Tunai QRIS</span>
                                                <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('qrisTarikTunai', val ? parseInt(val) : 0);" value="{{ $qrisTarikTunai ? number_format((float)$qrisTarikTunai, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="hover:bg-[#F3F6F4]/50 transition">
                                        <td class="py-2.5 px-3.5">
                                            ...
                                            <div class="text-[11px] text-[#718379] font-normal">M-Banking (Utama)</div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right">
                                            <div class="font-mono font-extrabold text-[#2C3E35] text-sm">Rp {{ number_format($mBcaAkhir, 0, ',', '.') }}</div>
                                        ...
                                        <td class="py-2.5 px-3.5 text-right whitespace-nowrap">
                                            @if($mBcaSelisih == 0)
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 rounded-md border border-emerald-200/80 inline-block">Sesuai</span>
                                            @else
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold font-mono text-rose-700 bg-rose-50 rounded-md border border-rose-200/80 inline-block">{{ $mBcaSelisih > 0 ? '+' : '' }}Rp {{ number_format($mBcaSelisih, 0, ',', '.') }}</span>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-3.5 text-center">
                                            <button type="button" x-on:click="openAdj = (openAdj === 'bca' ? null : 'bca')" class="px-2 py-1 text-[11px] font-extrabold rounded-lg transition cursor-pointer {{ ($bcaTopup > 0 || $bcaTrx > 0) ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 hover:bg-slate-200 text-[#718379]' }}">
                                                Top Up / Trx
                                            </button>
                                        </td>
                                    </tr>
                                    <tr x-show="openAdj === 'bca'" x-cloak class="bg-slate-50/80">
                                        <td colspan="5" class="p-3 border-t border-slate-200/60">
                                            <div class="grid grid-cols-2 gap-3 max-w-md ml-auto">
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Top Up Manual (BCA)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('bcaTopup', val ? parseInt(val) : 0);" value="{{ $bcaTopup ? number_format((float)$bcaTopup, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Trx Dipakai (BCA)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('bcaTrx', val ? parseInt(val) : 0);" value="{{ $bcaTrx ? number_format((float)$bcaTrx, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="hover:bg-[#F3F6F4]/50 transition">
                                        <td class="py-2.5 px-3.5">
                                            ...
                                            <div class="text-[11px] text-[#718379] font-normal">M-Banking (Ops)</div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right">
                                            <div class="font-mono font-extrabold text-[#2C3E35] text-sm">Rp {{ number_format($mBankmasAkhir, 0, ',', '.') }}</div>
                                        ...
                                        <td class="py-2.5 px-3.5 text-right whitespace-nowrap">
                                            @if($mBankmasSelisih == 0)
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 rounded-md border border-emerald-200/80 inline-block">Sesuai</span>
                                            @else
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold font-mono text-rose-700 bg-rose-50 rounded-md border border-rose-200/80 inline-block">{{ $mBankmasSelisih > 0 ? '+' : '' }}Rp {{ number_format($mBankmasSelisih, 0, ',', '.') }}</span>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-3.5 text-center">
                                            <button type="button" x-on:click="openAdj = (openAdj === 'bankmas' ? null : 'bankmas')" class="px-2 py-1 text-[11px] font-extrabold rounded-lg transition cursor-pointer {{ ($bankmasTopup > 0 || $bankmasTrx > 0) ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 hover:bg-slate-200 text-[#718379]' }}">
                                                Top Up / Trx
                                            </button>
                                        </td>
                                    </tr>
                                    <tr x-show="openAdj === 'bankmas'" x-cloak class="bg-slate-50/80">
                                        <td colspan="5" class="p-3 border-t border-slate-200/60">
                                            <div class="grid grid-cols-2 gap-3 max-w-md ml-auto">
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Top Up Manual (BANK MAS)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('bankmasTopup', val ? parseInt(val) : 0);" value="{{ $bankmasTopup ? number_format((float)$bankmasTopup, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Trx Dipakai (BANK MAS)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('bankmasTrx', val ? parseInt(val) : 0);" value="{{ $bankmasTrx ? number_format((float)$bankmasTrx, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="hover:bg-[#F3F6F4]/50 transition">
                                        <td class="py-2.5 px-3.5">
                                            ...
                                            <div class="text-[11px] text-[#718379] font-normal">Aplikasi PPOB</div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right">
                                            <div class="font-mono font-extrabold text-[#2C3E35] text-sm">Rp {{ number_format($mMultiAkhir, 0, ',', '.') }}</div>
                                        ...
                                        <td class="py-2.5 px-3.5 text-right whitespace-nowrap">
                                            @if($mMultiSelisih == 0)
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 rounded-md border border-emerald-200/80 inline-block">Sesuai</span>
                                            @else
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold font-mono text-rose-700 bg-rose-50 rounded-md border border-rose-200/80 inline-block">{{ $mMultiSelisih > 0 ? '+' : '' }}Rp {{ number_format($mMultiSelisih, 0, ',', '.') }}</span>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-3.5 text-center">
                                            <button type="button" x-on:click="openAdj = (openAdj === 'multi' ? null : 'multi')" class="px-2 py-1 text-[11px] font-extrabold rounded-lg transition cursor-pointer {{ ($multiTopup > 0 || $multiTrx > 0) ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 hover:bg-slate-200 text-[#718379]' }}">
                                                Top Up / Trx
                                            </button>
                                        </td>
                                    </tr>
                                    <tr x-show="openAdj === 'multi'" x-cloak class="bg-slate-50/80">
                                        <td colspan="5" class="p-3 border-t border-slate-200/60">
                                            <div class="grid grid-cols-2 gap-3 max-w-md ml-auto">
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Top Up Manual (MULTI)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('multiTopup', val ? parseInt(val) : 0);" value="{{ $multiTopup ? number_format((float)$multiTopup, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Trx Dipakai (MULTI)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('multiTrx', val ? parseInt(val) : 0);" value="{{ $multiTrx ? number_format((float)$multiTrx, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="hover:bg-[#F3F6F4]/50 transition">
                                        <td class="py-2.5 px-3.5">
                                            ...
                                            <div class="text-[11px] text-[#718379] font-normal">Agen Ekspedisi</div>
                                        </td>
                                        <td class="py-2.5 px-3.5 text-right">
                                            <div class="font-mono font-extrabold text-[#2C3E35] text-sm">Rp {{ number_format($mWahanaAkhir, 0, ',', '.') }}</div>
                                        ...
                                        <td class="py-2.5 px-3.5 text-right whitespace-nowrap">
                                            @if($mWahanaSelisih == 0)
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 rounded-md border border-emerald-200/80 inline-block">Sesuai</span>
                                            @else
                                                <span class="px-2 py-0.5 text-[11px] font-extrabold font-mono text-rose-700 bg-rose-50 rounded-md border border-rose-200/80 inline-block">{{ $mWahanaSelisih > 0 ? '+' : '' }}Rp {{ number_format($mWahanaSelisih, 0, ',', '.') }}</span>
                                            @endif
                                        </td>
                                        <td class="py-2.5 px-3.5 text-center">
                                            <button type="button" x-on:click="openAdj = (openAdj === 'wahana' ? null : 'wahana')" class="px-2 py-1 text-[11px] font-extrabold rounded-lg transition cursor-pointer {{ ($wahanaTopup > 0 || $wahanaTrx > 0) ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 hover:bg-slate-200 text-[#718379]' }}">
                                                Top Up / Trx
                                            </button>
                                        </td>
                                    </tr>
                                    <tr x-show="openAdj === 'wahana'" x-cloak class="bg-slate-50/80">
                                        <td colspan="5" class="p-3 border-t border-slate-200/60">
                                            <div class="grid grid-cols-2 gap-3 max-w-md ml-auto">
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Top Up Manual (WAHANA)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('wahanaTopup', val ? parseInt(val) : 0);" value="{{ $wahanaTopup ? number_format((float)$wahanaTopup, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                                <div>
                                                    <span class="text-[11px] text-[#718379] font-extrabold block mb-1">Trx Dipakai (WAHANA)</span>
                                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('wahanaTrx', val ? parseInt(val) : 0);" value="{{ $wahanaTrx ? number_format((float)$wahanaTrx, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-8 px-2.5 border border-slate-300 rounded-lg bg-white font-mono font-bold text-right text-xs focus:ring-1 focus:ring-[#3F7A5D]" />
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <!-- Tarik Tunai Kasir Toko & Catatan Rekap -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div class="bg-[#F3F6F4] p-3 rounded-2xl border border-slate-200/80 space-y-1.5">
                                <label class="block font-extrabold text-xs text-[#2C3E35]">Tarik Tunai Kasir Toko</label>
                                <div class="relative flex items-center h-9">
                                    <span class="absolute left-2.5 font-mono text-[11px] text-slate-400 select-none pointer-events-none">Rp</span>
                                    <input type="text" x-data x-on:input="let val = $el.value.replace(/\D/g, ''); $el.value = val ? parseInt(val).toLocaleString('id-ID') : ''; $wire.set('tarikTunaiKasir', val ? parseInt(val) : 0);" value="{{ $tarikTunaiKasir ? number_format((float)$tarikTunaiKasir, 0, ',', '.') : '' }}" placeholder="0" class="w-full h-full pl-7 pr-2.5 border border-slate-300 rounded-xl bg-white font-mono font-extrabold text-right text-xs text-[#2C3E35] focus:ring-2 focus:ring-[#3F7A5D]/20 focus:border-[#3F7A5D]" />
                                </div>
                                <span class="text-[11px] text-[#718379] font-medium block">Setoran tunai yang ditarik dari laci kasir.</span>
                            </div>

                            <div class="bg-[#F3F6F4] p-3 rounded-2xl border border-slate-200/80 space-y-1.5">
                                <label class="block font-extrabold text-xs text-[#2C3E35]">Catatan Rekap Harian</label>
                                <textarea wire:model="notes" rows="2" placeholder="Tuliskan penjelasan jika terdapat selisih..." class="w-full p-2 border border-slate-300 rounded-xl bg-white text-xs font-medium text-[#2C3E35] focus:ring-2 focus:ring-[#3F7A5D]/20 focus:border-[#3F7A5D] transition resize-none"></textarea>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-slate-100">
                        ...
                        <div class="flex items-center gap-2">
                            <button type="button" wire:click="saveInputModal" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition flex items-center gap-1.5 cursor-pointer active:scale-95">
                            ...
                            <button type="button" wire:click="validateAndLock" wire:loading.attr="disabled" class="px-4 py-2 bg-[#3F7A5D] hover:bg-[#32634B] text-white font-extrabold text-xs rounded-xl shadow-xs transition flex items-center gap-1.5 cursor-pointer active:scale-95">
